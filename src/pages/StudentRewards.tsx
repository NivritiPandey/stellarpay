...
export const StudentRewards: React.FC = () => {
  ...
  return (
    <div className="space-y-stack-lg animate-fade-in-up text-gray-900 dark:text-white pb-8">
      ...
      {isProcessing && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex flex-col items-center justify-center pointer-events-none">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-stack-lg mb-12">
        <div className="max-w-2xl">
          ...
          <p className="font-body-lg text-body-lg text-gray-600 dark:text-on-surface-variant">Unlock exclusive academic perks and digital assets by completing educational milestones and participating in the Stellar ecosystem.</p>
        </div>
        <div className="glass-card p-6 rounded-3xl flex items-center gap-6 shadow-xl border-gray-200 dark:border-white/5 min-w-[280px]">
          ...
          <div>
            <p className="font-label-sm text-label-sm text-gray-500 dark:text-on-surface-variant uppercase tracking-widest">Global Earnings</p>
            <p className="font-headline-lg text-headline-lg font-bold text-gray-900 dark:text-white">12.8M <span className="text-primary font-normal text-headline-md">PTS</span></p>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
        ...
        <section className="md:col-span-4 flex flex-col gap-gutter">
          <div className="glass-card p-stack-lg rounded-[32px] flex flex-col gap-6 relative overflow-hidden group border-gray-200 dark:border-white/5">
            ...
            <div className="flex items-center justify-between">
              <div className="w-20 h-20 rounded-2xl overflow-hidden border-2 border-primary/30 bg-gray-100 dark:bg-surface-container-low flex-shrink-0">
            ...
            <div>
              <h3 className="font-headline-md text-headline-md text-gray-900 dark:text-white">{studentName}</h3>
              <p className="text-gray-500 dark:text-on-surface-variant font-label-md text-label-md">Computer Science • Junior</p>
            </div>

            <div className="pt-4 border-t border-gray-200 dark:border-white/5">
              <div className="flex justify-between items-end mb-2">
                <p className="font-label-sm text-label-sm text-gray-500 dark:text-on-surface-variant">Wallet Balance</p>
              ...
              <div className="flex items-baseline gap-2">
                <span className="font-headline-lg text-3xl font-bold text-gray-900 dark:text-white" id="points-display">{displayPoints}</span>
                <span className="text-primary font-bold">PTS</span>
              </div>
              <div className="w-full h-1.5 bg-gray-250 dark:bg-white/5 rounded-full mt-4 overflow-hidden">
                <div className="h-full primary-gradient rounded-full" style={{ width: `${Math.min((displayPoints / 500) * 100, 100)}%` }}></div>
              </div>
              <p className="mt-2 font-label-sm text-label-sm text-gray-500 dark:text-on-surface-variant text-right">
          ...
          {/* Action Panel */}
          <div className="glass-card p-stack-lg rounded-[32px] flex flex-col gap-4 border-gray-200 dark:border-white/5">
            <h4 className="font-label-md text-label-md font-bold uppercase tracking-widest text-gray-500 dark:text-on-surface-variant mb-2">Rewards Actions</h4>
            
            {showRegisterInput ? (
              <div className="flex flex-col gap-2 p-3 bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-250 dark:border-white/10">
                <input
                  type="text"
                  placeholder="Enter Student Name"
                  value={inputName}
                  onChange={(e) => setInputName(e.target.value)}
                  className="bg-transparent border border-gray-300 dark:border-white/10 rounded-xl px-3 py-2 text-sm w-full placeholder:text-gray-400 dark:placeholder:text-on-surface-variant/40 text-gray-900 dark:text-white focus:outline-none focus:border-primary"
                />
                <div className="flex gap-2 justify-end">
                  <button
                    onClick={() => setShowRegisterInput(false)}
                    className="px-3 py-1.5 bg-gray-250 dark:bg-white/5 rounded-lg text-xs"
                  >
                    Cancel
                  </button>
                  <button
              ...
              <button
                ...
                className="w-full py-4 px-6 rounded-2xl bg-gray-150 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 border border-gray-300 dark:border-white/10 text-gray-900 dark:text-white font-bold font-label-md text-label-md flex items-center justify-center gap-3 transition-all active:scale-95"
              >
            ...
            <p className="text-[11px] text-center text-gray-500 dark:text-on-surface-variant mt-2 px-4">
        ...
        <section className="md:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-gutter">
          ...
          <div className="md:col-span-2 glass-card rounded-[32px] overflow-hidden flex flex-col border-gray-200 dark:border-white/5">
            <div className="p-stack-lg border-b border-gray-200 dark:border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-primary">history</span>
                <h3 className="font-headline-md text-headline-md text-gray-900 dark:text-white font-bold">Reward History</h3>
              </div>
              <button className="text-primary font-label-md text-label-md hover:underline decoration-2 underline-offset-4 font-bold">View All</button>
            </div>
            
            <div className="flex-1 max-h-[400px] overflow-y-auto custom-scrollbar">
              <table className="w-full text-left">
                <thead className="sticky top-0 bg-gray-50 dark:bg-[#10131a] z-10">
                  <tr className="text-gray-500 dark:text-on-surface-variant font-label-sm text-label-sm uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-white/5">
                  {milestones.map((m) => {
                    ...
                    return (
                      <tr 
                        key={m.id} 
                        className={`hover:bg-gray-100 dark:hover:bg-white/5 transition-colors group cursor-pointer ${isClaimed ? 'opacity-70' : ''}`}
                        onClick={() => {
                      ...
                      >
                        <td className="px-stack-lg py-5 flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            ...
                            m.icon === 'groups' ? 'bg-primary/10 text-primary' : 'bg-gray-150 dark:bg-white/5 text-gray-500 dark:text-on-surface-variant'
                          }`}>
                            <span className="material-symbols-outlined">{m.icon}</span>
                          </div>
                          <div>
                            <p className="font-label-md text-label-md text-gray-900 dark:text-white font-bold">{m.title}</p>
                            <p className="text-[12px] text-gray-500 dark:text-on-surface-variant">{m.category}</p>
                          </div>
                        </td>
                        <td className="px-stack-lg py-5 text-gray-500 dark:text-on-surface-variant font-label-sm text-label-sm">{m.date}</td>
                        <td className="px-stack-lg py-5">
                          {isClaimed ? (
                            <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-600 dark:text-green-400 font-label-sm text-label-sm border border-green-500/20">Sent</span>
                          ) : isLocked ? (
          ...
          <div className="glass-card p-6 rounded-[32px] flex flex-col gap-4 relative overflow-hidden group border-gray-200 dark:border-white/5">
            <div className="w-full h-40 rounded-2xl bg-cover bg-center mb-2 bg-gray-100 dark:bg-[#191c22]" style={{ backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuBXQxUjVg8Bb92aVJt9hmLfKLz6tS4lIaEkAWRFeOee6UH6JJ7haOuWuffsSgpZNuOqlS0cmKbfoeI9u4iqY9bmghxBXYafr4NXuLgseAt1C8cHfgrmQwFggN4yUBq8lsopkHiFIx0GwfTgIwq6DhG5aMR1YVT__gFUJq5jldLogPTeHNzzMVjVER-NUjSQgFC8GOlMwk1IO06XWicMgY5wdxuZObb8C_Bd4_hxbHQaKMH9xvKK9la3iZnsAtwAt4vY8xj9LMVjEiQ')" }}></div>
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-headline-md text-headline-md text-gray-900 dark:text-white font-bold">Digital Mentor NFT</h4>
            ...
            <p className="text-gray-500 dark:text-on-surface-variant text-sm line-clamp-2">Exclusive access to the private developer workshop series and a unique Soulbound token.</p>
            <button
              onClick={() => handleClaimReward(2500, 'Digital Mentor NFT')}
              className="mt-auto py-3 rounded-xl bg-gray-150 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 border border-gray-300 dark:border-white/10 text-gray-900 dark:text-white font-bold transition-all active:scale-95"
            >
              Unlock Item
            </button>
          </div>
          ...
          {/* Program Stats Card */}
          <div className="glass-card p-6 rounded-[32px] flex flex-col justify-between border-gray-200 dark:border-white/5 text-gray-900 dark:text-white">
            <div>
              <h4 className="font-label-md text-label-md font-bold uppercase tracking-widest text-gray-500 dark:text-on-surface-variant mb-6">Program Impact</h4>
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-headline-md font-bold text-gray-900 dark:text-white">450+</p>
                    <p className="text-label-sm text-gray-500 dark:text-on-surface-variant">Registered Institutions</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-headline-md font-bold text-gray-900 dark:text-white">1.2M XLM</p>
                    <p className="text-label-sm text-gray-500 dark:text-on-surface-variant">Distributed in Grants</p>
                  </div>
                </div>
              </div>
            </div>
